...
import { EvaluationResponseView } from "@/lib/evaluations/response-view";
import type { HotEvaluationData } from "@/lib/evaluations/hot";

const UUID_REGEX =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

export default async function EvaluationDetailAdminPage({
  params,
}: {
  ...
  const { data: enrollment } = await supabase
    ...
    .select(
      "id, session_id, learner:learners(civility, first_name, last_name, company:companies(name))",
    )
    .eq("id", enrollmentId)
    .maybeSingle<{
      id: string;
  ...
  const { data: response } = await supabase
    ...
    .maybeSingle<{ data: HotEvaluationData; submitted_at: string }>();
    ...
    <>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: "Évaluation", href: `/sessions/${id}/evaluation` },
          { label: learnerName },
        ]}
        actions={<BackButton fallbackHref={`/sessions/${id}/evaluation`} />}
      />
        ...
        {response ? (
          ...
          <div className="rounded-xl bg-amber-50 border border-amber-200 p-6 text-center">
            <p className="text-sm text-amber-900 font-medium">
              ⏳ Évaluation non encore remplie par cet apprenant.
            </p>
            <p className="text-xs text-amber-700 mt-1">
              L&apos;apprenant peut la compléter via le QR projeté en fin
              de séance ou via son portail personnel.
            </p>
          </div>
        )}
      </div>
