...
import {
  getOrCreateSessionQrToken,
  regenerateSessionQrToken,
  type SessionQrTokenResult,
} from "../actions";
...
/**
 * Bouton "QR code émargement" + modale plein écran.
 *
 * Au clic : génère (ou récupère) le token actif de la session, affiche
 * un QR code que le formateur peut projeter aux apprenants. Les apprenants
 * scannent et accèdent à /emarger/[token] sur leur téléphone pour signer.
 */
export function QrButton({ sessionId }: Props) {
  const [open, setOpen] = useState(false);
  const [fullscreen, setFullscreen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<SessionQrTokenResult | null>(null);
  const [mounted, setMounted] = useState(false);

  // createPortal nécessite que le DOM existe (post-hydration)
  useEffect(() => {
    setMounted(true);
  }, []);

  // Charge le token quand on ouvre la modale
  useEffect(() => {
    if (!open || data) return;
    let cancelled = false;
    setLoading(true);
    getOrCreateSessionQrToken(sessionId).then((res) => {
      if (cancelled) return;
      setData(res);
      setLoading(false);
    });
  ...
  async function handleRegenerate() {
    if (!confirm("Régénérer un nouveau QR code ? L'ancien sera invalidé.")) return;
    setLoading(true);
    const res = await regenerateSessionQrToken(sessionId);
    setData(res);
    setLoading(false);
  }
  ...
  const overlays = (
    <>
          ...
          >
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-sm text-zinc-600">
                Les apprenants scannent avec leur téléphone, choisissent
                leur nom puis signent matin / après-midi.
              </p>
            </div>
      ...
      {/* Plein écran : QR géant + URL en bas, pour projeter au vidéoprojecteur */}
      {open && fullscreen && data?.publicUrl && (
        ...
        >
          <div className="text-xs uppercase tracking-widest text-zinc-500 font-semibold text-center shrink-0">
            Émargement en ligne — Scannez avec votre téléphone
          </div>
          ...
          <div className="shrink-0 text-center space-y-1">
            <a
              ...
              rel="noopener noreferrer"
              onClick={(e) => e.stopPropagation()}
              className="text-sm text-cyan-700 hover:underline font-mono break-all px-2"
            >
              {data.publicUrl}
            </a>
