...
import {
  getOrCreateSessionEvaluationToken,
  regenerateSessionEvaluationToken,
  type SessionEvaluationTokenResult,
...
/**
 * Bouton "QR code évaluation à chaud" + modale plein écran.
 * Le formateur projette ce QR en fin de séance pour que chaque
 * apprenant remplisse l'évaluation Qualiopi sur son téléphone
 * avant de quitter la salle.
 */
export function QrEvaluationButton({ sessionId }: Props) {
  const [open, setOpen] = useState(false);
  const [fullscreen, setFullscreen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<SessionEvaluationTokenResult | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    getOrCreateSessionEvaluationToken(sessionId).then((res) => {
      if (cancelled) return;
      setData(res);
      setLoading(false);
    });
  ...
  async function handleRegenerate() {
    ...
    setLoading(true);
    const res = await regenerateSessionEvaluationToken(sessionId);
    setData(res);
    setLoading(false);
  }
