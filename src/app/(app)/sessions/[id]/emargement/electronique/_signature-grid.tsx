...
type Props = {
  ...
  /** Libellé court de la modalité affiché sous chaque date dans le thead. */
...
/** Clé unique pour identifier une signature dans la map locale. */
function sigKey(
...
type ModalState = {
  enrollmentId: string | null; // null = signature formateur
  signerName: string;
  signerRole: SignerRole;
  periodDate: string;
  moment: Moment;
  /** ID d'enrollment "ancrage" pour les signatures formateur (la table
   *  attendance_signatures a `enrollment_id NOT NULL`, donc on ancre la
   *  signature formateur sur l'enrollment du 1er apprenant). */
  trainerAnchorEnrollmentId?: string;
} | null;
...
export function SignatureGrid({
  ...
  // Map locale des signatures (clé → SignatureSnapshot)
  ...
  // Pour l'ancrage des signatures formateur : on prend l'enrollment du
  // 1er apprenant. Si aucun apprenant, le bouton formateur est désactivé.
  ...
  function openSignerModalForTrainer(date: string, moment: Moment) {
    ...
    setModal({
      ...
      periodDate: date,
      moment,
      trainerAnchorEnrollmentId: trainerAnchor,
    });
  }
  ...
  function handleValidate() {
    ...
    // Optimistic update : on insère immédiatement dans la map locale
    setSignatures((prev) => {
    ...
    startTransition(async () => {
      try {
        ...
        alert(
          "Échec de l'enregistrement de la signature. Vérifiez votre connexion.",
        );
      } finally {
        setPending((prev) => {
          const copy = { ...prev };
  ...
  function handleClear(
    ...
    startTransition(async () => {
      try {
        ...
        console.error("clearSignature error:", e);
        if (previous) {
          setSignatures((prev) => {
            const next = new Map(prev);
        ...
        alert("Échec de la suppression de la signature.");
      } finally {
        setPending((prev) => {
          const copy = { ...prev };
          delete copy[key];
                      ...
                      >
                        {sig ? (
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              openSignerModalForLearner(l, p.date, m)
                            }
                            disabled={isPending}
                    ...
                    >
                      {sig ? (
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            openSignerModalForTrainer(p.date, m)
                          }
