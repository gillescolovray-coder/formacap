...
import { setAttendanceAsTrainer } from "../actions";
...
/**
 * Variante trainer de AttendanceGrid (admin). Pointage manuel
 * présent/absent/excusé/retard. Auth par token portail formateur.
 */
export function TrainerAttendanceGrid({
  token,
  sessionId,
  periods,
  rows,
      ...
      for (const row of rows) {
        for (const p of periods) {
          for (const moment of MOMENTS) {
            ...
            m[localKey] = row.attendancesByKey[dataKey] ?? "not_recorded";
          }
        }
      }
      return m;
  ...
  function handleChange(
    ...
    startTransition(async () => {
      try {
        ...
        console.error("setAttendanceAsTrainer failed, rolling back:", e);
              ...
              <td className="px-4 py-3 sticky left-0 bg-white dark:bg-zinc-900 z-10 border-r border-zinc-200 dark:border-zinc-800">
                ...
                {row.partnerOfName && (
                  <div
                    ...
                    title={`Inscrit via ${row.partnerOfName}.`}
                  >
