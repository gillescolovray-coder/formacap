...
import {
  getOrCreateSessionQrTokenAsTrainer,
...
/**
 * Variante trainer du bouton QR code émargement.
 * Auth par token portail formateur (cf. actions trainer).
 */
export function TrainerQrButton({ token, sessionId }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    getOrCreateSessionQrTokenAsTrainer(token, sessionId).then((res) => {
      if (cancelled) return;
      setData(res);
      setLoading(false);
    });
  ...
  async function handleRegenerate() {
    ...
    setLoading(true);
    const res = await regenerateSessionQrTokenAsTrainer(token, sessionId);
    setData(res);
    setLoading(false);
  }
