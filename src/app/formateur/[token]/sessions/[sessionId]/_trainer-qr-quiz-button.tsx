...
/**
 * Bouton "QR code quiz pré/post" — variante portail formateur.
 *
 * Le formateur projette ce QR : un seul QR par session, chaque
 * apprenant scanne, choisit son nom et joue son quiz d'entrée puis
 * (à la fin) son quiz de sortie. L'anti-rejeu (1 fois pre, 1 fois
 * post) est géré côté /mon-parcours/[token]/quiz.
 *
 * Couleur amber pour rester cohérent avec le module Quiz (Module 5
 * bis du portail formateur).
 */
export function TrainerQrQuizButton({ token, sessionId }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    getOrCreateSessionQuizTokenAsTrainer(token, sessionId).then((res) => {
      if (cancelled) return;
      setData(res);
      setLoading(false);
    });
  ...
  async function handleRegenerate() {
    if (!confirm("Régénérer un nouveau QR code ? L'ancien sera invalidé."))
      return;
    setLoading(true);
    const res = await regenerateSessionQuizTokenAsTrainer(token, sessionId);
    setData(res);
    setLoading(false);
  }
            ...
            >
              ...
              <div className="text-center space-y-1">
                ...
                <p className="text-sm text-zinc-600">
                  Choisissez votre nom dans la liste puis répondez aux
                  questions. Quiz d&apos;entrée le matin, quiz de sortie en
                  fin de session.
                </p>
              </div>
      ...
      {open && fullscreen && data?.publicUrl && (
        ...
        >
          <div className="text-xs uppercase tracking-widest text-amber-700 font-semibold text-center shrink-0">
            Quiz d&apos;évaluation — Scannez avec votre téléphone
          </div>
